...
for item in container.query_items(query="SELECT * FROM c WHERE c.type = 'candidate'", enable_cross_partition_query=True):
    ...
    github_link = None
    if resume:
        # Try to find a github.com substring in the raw resume text
        if 'github.com' in resume:
            # Try to extract the first github.com URL
            import re
            matches = re.findall(r'(https?://github\.com/[^\s\"\}\],]+)', resume)
            if matches:
                github_link = matches[0]
        # Otherwise, try to parse as JSON and extract links.gitHub
        if not github_link:
            import json
            try:
